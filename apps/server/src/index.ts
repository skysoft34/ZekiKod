...
const geminiLiveService = new GeminiLiveService(agentService, DATA_DIR);
...
server.on('upgrade', (request, socket, head) => {
  ...
  } else if (pathname === '/api/gemini-live/ws' || pathname === '/gemini-live/ws') {
...
// Gemini Live WebSocket connection handler
geminiLiveWss.on('connection', (ws: WebSocket) => {
  logger.info('Gemini Live client connected');
  const apiKey = process.env.GOOGLE_GENAI_API_KEY;

  if (!apiKey) {
    logger.error('No Google GenAI API Key found');
    ws.close(1008, 'Server key configuration missing');
    return;
  }

  geminiLiveService.connect(ws, apiKey);
});
