...
export class AgentService {
  ...
  async startConversation({
    ...
    if (!this.sessions.has(sessionId)) {
      ...
      const effectiveWorkingDirectory = workingDirectory || sessionMetadata?.workingDirectory || sessionMetadata?.projectPath || process.cwd();
  ...
  async sendMessage({
    ...
    if (imagePaths && imagePaths.length > 0) {
      for (const imagePath of imagePaths) {
        try {
          ...
          const base64Content = imageData.base64?.trim();

          if (base64Content && base64Content.length > 100) { // Require at least 100 chars (valid image is much larger)
            images.push({
              data: base64Content,
            ...
            this.logger.debug(`Loaded image ${imagePath} with ${base64Content.length} chars of base64 data.`);
          } else {
            this.logger.warn(`Skipping invalid/small image: ${imagePath} (length: ${base64Content?.length || 0})`);
          }
        } catch (error) {
    ...
    try {
      ...
      // Build prompt content manually 
      let promptContent: any;

      // Process images if available
      if (images.length > 0) {
        // Construct multi-part content for Claude/Providers supporting vision
        ...
        // 2. Add text message with image paths for context
        let textMessage = message;
        if (validImagePaths.length > 0) {
          textMessage += '\n\nAttached Images:\n' + validImagePaths.map(p => `- ${p}`).join('\n');
        }
      ...
      // Note: Cast to any/string because ExecuteOptions might define prompt strictly as string,
      // but the provider implementation checks for Array.isArray(prompt).
      options.prompt = promptContent as any;
      ...
      let lastSaveTime = 0;
      const SAVE_INTERVAL_MS = 2000; // Save every 2 seconds during streaming
          ...
          if (msg.message?.content) {
            for (const block of msg.message.content) {
              if (block.type === 'text') {
                ...
                // Periodic save to persist partial responses in case of crash
                const now = Date.now();
                if (now - lastSaveTime > SAVE_INTERVAL_MS) {
                  lastSaveTime = now;
                  // Don't await this to avoid blocking the stream
                  this.saveSession(sessionId, session.messages).catch(err =>
                    this.logger.error('Failed to auto-save session during stream:', err)
                  );
                }
