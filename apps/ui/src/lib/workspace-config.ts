...
/**
 * Gets the default Documents/ZekiKod directory path
 * @returns Promise resolving to Documents/ZekiKod path, or null if unavailable
 */
async function getDefaultDocumentsPath(): Promise<string | null> {
  try {
    ...
    if (typeof window !== 'undefined' && (window as any).electronAPI?.getPath) {
      ...
      return joinPath(documentsPath, 'ZekiKod');
    }
 ...
 * 3. Documents/ZekiKod (if ALLOWED_ROOT_DIRECTORY is not set)
