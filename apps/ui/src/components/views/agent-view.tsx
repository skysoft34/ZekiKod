...
import { NoProjectState, AgentHeader, ChatArea, ThinkingPanel } from './agent-view/components';
import { AgentInputArea } from './agent-view/input-area';

export function AgentView() {
  const { currentProject } = useAppStore();
  // Input state moved to AgentInputArea for performance
  ...
  const handleSend = useCallback(async (content: string) => {
    ...
    if (!content.trim() && selectedImages.length === 0 && selectedTextFiles.length === 0) return;
  ...
  }, [fileAttachments, isProcessing, sendMessage, addToServerQueue]);
      ...
      ? [
        {
          ...
          content:
            "Hello! I'm the ZekiKod Agent. I can help you build software autonomously. I can read and modify files in this project, run commands, and execute tests. What would you like to create today?",
